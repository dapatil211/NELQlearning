...
def get_epsilon(i, epsilon, EPS_DELTA, EPS_START, EPS_END, EPS_DECAY_START):
...
def train(agent, env, actions, optimizer):
    ...
    def eps_func(i, epsilon):
        return get_epsilon(i, epsilon, EPS_DELTA, EPS_START, EPS_END, EPS_DECAY_START)
    ...
    sr_window = train_config['self_reflect_window']
    sr_update_interval = train_config['self_reflect_update_interval']
    sr_epsilon_value = train_config['self_reflect_epsilon_value']
    ...
    painter = None
    sr_losses = deque(maxlen=sr_window)
    sr_rewards = deque(maxlen=sr_window*2)
    ...
    for training_steps in range(max_steps):
        ...
        # with time.
        if training_steps % sr_update_interval==0 and training_steps > EPS_DECAY_START:
            m_loss, _, _, _, _ = linregress(range(len(sr_losses)), list(sr_losses))
            avg_rewards = np.mean(sr_rewards)
            avg_total_rewards = np.mean(reward_list)
            print('m_loss ', m_loss)
            print('avg_rewards ', avg_rewards)
            print('avg_total_rewards ', avg_total_rewards)

            if avg_rewards < avg_total_rewards and m_loss < 0:
                print('updated****************************', sr_epsilon_value)
                
                epsilon = sr_epsilon_value
    
        epsilon = eps_func(training_steps, epsilon)
