...
def train(agent, env, actions, optimizer):
    ...
    def eps_func(i, epsilon):
        return get_epsilon(i, epsilon, EPS_START, EPS_END, EPS_DECAY_START, EPS_DECAY_END)
    ...
    all_rewards_trigger_needed = deque(maxlen=200)
    rewards = []
    ...
    def trigger_epsilon_change():
        temp = list(all_rewards_trigger_needed)
        temp = [ sum(temp[i:i+99]) for i in range(100)]
        slope, _, _, _, _ = linregress(range(100), temp)
    ...
    for training_steps in range(max_steps):
        ...
        # with time.
        if (training_steps % UPDATE_INTERVAL == 0) and (len(all_rewards_trigger_needed) == 200):
            trigger = trigger_epsilon_change()
            if trigger:
                epsilon = max(epsilon, EPSILON_MAX)
                print("****self-reflection triggered, epsilon=", epsilon, "****")
            else:
        ...
        all_rewards_trigger_needed.append(reward)
